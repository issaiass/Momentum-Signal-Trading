...
| `MARKET_WIDE_NEGATIVE_MOMENTUM_CASH` | WARNING | `execution/live_signal.py::run()` | `use_negative_universe_cash_filter` is on AND every ticker in the eligible universe has non-positive trailing momentum; holds literal cash instead of the "least bad" `top_n` or a `defensive_ticker` swap. Fires alongside `NO_ELIGIBLE_TICKERS` (both mean different things, see `docs/RISK_CONSTRAINTS.md`'s "Whole-Book Negative Momentum Cash Filter") but never alongside `INSUFFICIENT_PRICE_HISTORY` (scores must have been computed for this check to even run) |
| `LOG_ROTATED` | INFO | `daily_runner.py::_rotate_log_with_alert()` | `enable_log_retention` is on for at least one portfolio and a rotation actually moved rows out of a log into an archive file; names the log, row counts, and archive file. See `docs/LOG_RETENTION.md` |
| `CONFIG_CHANGED` | INFO | `daily_runner.py::detect_and_log_config_change()` | This portfolio's effective config differs from the previous run's (a real edit to `config.yaml` took effect); names every changed field as `field: old -> new`. Never fires on a portfolio's very first run (nothing to diff against yet). See `docs/DEPLOYMENT.md`'s "What needs what" |
