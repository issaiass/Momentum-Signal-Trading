...
504 tests should pass cleanly. This only confirms code mechanics work on this machine
...
`risk_monitor.py` in a **separate container or separate host**, pointed at BOTH the `logs/`
volume (read-only mount recommended, this is where `live_trades_log_<portfolio>.csv` actually
lives, `--log-dir` defaults to `logs_dir()`) AND the `data/` volume (needs write access here,
this is where it writes the halt flag `daily_runner.py` checks). These are two different
directories, don't mount only one.
