...
| `backtest/test_momentum_backtest.py` | Core backtest engine: `BacktestConfig` validation (including `allow_fractional_shares`, `top_n`, fractional `holding_period`, fractional `lookback_period` for short-term/weekly momentum configs, the three risk-constraint fields `max_turnover_pct`/`skip_month_guardrail`/`position_vol_budget`, `total_value_drift_warning_pct`/`persist_dry_run_state`, `use_trailing_stop`/`attach_broker_trailing_stop`'s shared `trailing_stop_pct`-required validation, independent of each other), the shared `resolve_target_weights()` sizing path including `_apply_volatility_budget_caps()`'s per-ticker vol-budget cap (confirmed complementary to, not overridden by, the flat `max_position_weight` cap), crash-protection mechanisms (circuit breaker, correlation spike detection, liquidity stress), and (Epic 14) `momentum_crash_lookback_days`/`momentum_crash_derate` validation plus a fabricated bear+high-vol price panel confirming the joint condition fires only when both halves are true at once, not from either alone |
| `execution/test_live_signal.py` | Live order generation (BUY/SELL/HOLD sizing), real FIFO P&L math, multi-portfolio orchestration, `get_top_etfs()`'s `top_n` selection behavior, the live-trading equivalents of backtest-only risk fields: `compute_aggregate_drift()`, `derive_entry_date()`, correlation-spike exposure scaling, `place_orders_ibkr()`'s sells-before-buys sequencing, cash-aware buy sizing, whole-share flooring for fractional orders (IBKR has no fractional equity API support), that orders dropped before ever reaching IBKR (flooring to 0 shares, cash-scaling to 0 shares) are still recorded in the returned results as `DROPPED_FRACTIONAL`/`DROPPED_INSUFFICIENT_CASH` rather than silently omitted, and that informational IBKR order-status codes never corrupt a real order's tracked status (mocked IBKR, no real broker needed), and that each wired alert point actually appends the right row to the alert log, plus `is_rebalance_day()`'s monthly and weekly-granularity scheduling logic, the `is_holding_period_too_frequent()`/`is_lookback_period_too_short()` threshold boundaries, `resolve_momentum_scores()`'s hand-verified weekly-vs-monthly momentum ranking arithmetic (all four documented short-term lookback pairs, a regression check that the monthly branch matches the pre-existing computation exactly, an integration test confirming `run()` actually wires `holding_period` through, and the opt-in `skip_month_guardrail`'s monthly-window shift, its no-op cases at `lookback_period <= 3` and in the weekly regime), the `docs/RISK_CONSTRAINTS.md` advisory checks `is_lookback_shorter_than_holding()`/`is_lookback_to_holding_ratio_too_low()`/`is_turnover_too_high()` and `compute_turnover()`'s hand-computed position-count ratio, and `build_position_performance()`'s return-since-entry calc, omission rules (missing entry price/zero shares/unknown live price), graceful handling of an undeterminable entry date, `most_recent_rebalance_target_date()`'s holiday-aware backward search for a missed rebalance day (monthly and weekly, and that it only searches strictly before today), `run()`'s `extra_price_tickers` param (widens pricing without widening the ranking/selection universe, and the default-omitted regression), and `reconstruct_dry_run_positions()`'s reuse of `measure_live_performance()`'s FIFO math (weighted-average cost across multiple buys, a fully-closed position being absent, a missing log returning `{}`). Also: Rank Delta (`resolve_lookback_period_row_count()`'s regime parity with `resolve_momentum_scores()`, `compute_required_lookback_days()`'s widened fetch when `use_rank_delta` is on, and `run()`'s hand-computed up/down/flat/insufficient-history cases), `compute_stop_loss_price()`'s per-share reporting formula (BUY uses close price, HOLD uses real `avg_entry_price` when known else falls back to close, per-ticker override resolution), and `place_orders_ibkr()`'s broker-side protective brackets: the fixed STP bracket, the broker-native TRAIL bracket (`attach_broker_trailing_stop`, including the real IBKR constraint that a TRAIL child requires its parent to be `LMT` not `MKT`), their One-Cancels-All pairing when both are enabled together (shared `ocaGroup`, only the last child transmits), and the cancel-before-sell mechanism now matching a resting `TRAIL` order the same way it already matched `STP`. Also (Epic 14): `momentum_crash_lookback_days`/`momentum_crash_derate`'s live-side wiring in `compute_target_weights()` (the extra derate applying only in the joint bear+high-vol regime, not from either half alone, and `None`'s byte-identical default), `compute_required_lookback_days()`'s widened fetch when this field is set, and `IBKR_INFORMATIONAL_CODES` including the newly-catalogued `2109` |
| `test_daily_runner.py` | CLI/operational layer: `config.yaml` schema validation, idempotent rebalance locking, alert fallback, circuit-breaker persistence-across-runs, the live time-based stop (`check_and_handle_time_stops`) and price-based stop-loss (`check_and_handle_stop_losses`), multi-portfolio capital safety on a shared IBKR account (`resolve_total_values()`'s remainder math, `check_ticker_overlap()`), `send_warning`/`send_email_command_feedback` config-type validation, the `ALERTS_REPORT` email command's end-to-end read-and-reply path, the `send_email_command_feedback` flag gating reply emails without blocking command application, per-command apply-time `ERROR` isolation (one command failing to apply does not abort the rest of the batch), the same-inbox visibility warning plus `--test-email`'s exit-code/no-config-load behavior, `lookback_period` resolving independently per portfolio via `risk_overrides`, `already_ran_today()`'s `as_of` override, `has_run_on_or_after()`'s range check backing the `MISSED_REBALANCE_DAY` warning (including the exact scenario a manual `--force-rebalance` catch-up must clear on the next run), the rebalance-in-progress marker's write/clear lifecycle and atomic-write guarantee, `_classify_orphaned_tickers()`'s confirmed-orphaned-vs-unrecognized partitioning (including the currently-flat-in-the-log safe-failure case), and `_compute_scoped_positions_value()`'s explicit ticker-intersection scoping (including a hand-verified proof it does NOT double-count a ticker shared between two portfolios, unlike the pre-existing `positions_value` computation) |
...
| `test_governance.py` | Institutional governance: VaR/CVaR, scenario shocks, capacity checks, tamper-evident hash-chained audit log, independent `risk_monitor.py` (including its `config.yaml` capital fallback and, critically, `main()`'s own default `--log-dir` actually finding the real trade log at `logs_dir()`, not `data_dir()`, a regression test for a real bug this project's default Docker cron invocation had, silently unable to ever halt trading), config-approval gate |
...
Current count: **935 tests**, all passing. Every test file and class has a docstring explaining
...
Worth repeating because it's easy to forget once a suite is green: **935 passing tests confirm
