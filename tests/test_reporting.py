...
class TestSignalContextInOrders:
    ...
    def test_transaction_amount_reaches_the_log(self, tmp_path):
        # transaction_amount (Epic 3): the actual $ bought this transaction (shares * price),
        # distinct from money_invested's target-allocation meaning above.
        path = str(tmp_path / "log.csv")
        cfg = BacktestConfig(min_trade_size=1.0)
        orders = generate_orders(
            current_holdings={}, target_weights={"XLK": 1.0}, gross_exposure=1.0,
        ...
        import pandas as pd
        df = pd.read_csv(path)
        assert df.iloc[0]["transaction_amount"] == pytest.approx(orders["XLK"]["shares"] * 220.0)
